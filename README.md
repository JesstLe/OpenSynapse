...
> 🎯 **OpenSynapse** 是一个开源的 AI 驱动知识管理系统，将 Gemini AI 对话、智能笔记提取、FSRS 间隔重复复习和 D3.js 知识图谱整合到一个工作流中。支持自托管架构、多设备访问、OAuth 认证，以及 CLI 命令行工具。
...
### ☁️ 云端同步与多租户
- **多登录支持** - Google、GitHub、Discord OAuth + 邮箱/密码登录，完整数据隔离
- **账号绑定** - 一个账号可绑定多种登录方式，灵活切换
- **用户级 API Key** - 商业部署支持每个用户使用独立的 API Key
- **自托管架构** - better-auth + PostgreSQL + Chroma，完全数据掌控
...
│                        Service Layer                            │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐  │
│  │ AI Gateway   │  │    FSRS      │  │  PostgreSQL/Chroma  │  │
│  │ (多提供商)    │  │  (间隔重复)   │  │  (数据/向量存储)      │  │
│  └──────────────┘  └──────────────┘  └──────────────────────┘  │
├─────────────────────────────────────────────────────────────────┤
│                        Library Layer                            │
...
│                      External Services                          │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐  │
│  │  PostgreSQL  │  │   Gemini     │  │   Google Cloud      │  │
│  │  better-auth │  │    API       │  │     OAuth           │  │
│  └──────────────┘  └──────────────┘  └──────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
